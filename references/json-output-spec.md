...
7.  **【致命错误防范】引号与转义规范**：AI 生成 JSON 时常因引号转义（`\"`）错误导致 JSON 解析失败和无限循环重试。为从根本上避免此问题，请**严格遵守**：
    - **中文字段**（`_zh`）：所有需要引用或强调的词语，**必须直接使用全角中文弯引号 `“...”` 和 `‘...’`**。**绝对禁止**在中文字段中使用英文直引号 `"`，也**不要**尝试使用 `\"` 进行转义。
      - ✅ 正确：`"intro_zh": "把“迁移到云”和“离开 Oracle”这两件事拆分"`
      - ❌ 错误（易致崩溃）：`"intro_zh": "把\"迁移到云\"和\"离开 Oracle\"这两件事拆分"`
    - **英文字段**（`_en`）：如果需要引用，**强烈建议使用单引号 `'...'`**，以避免双引号嵌套导致的转义风险。
      - ✅ 推荐：`"intro_en": "It's a 'game changer'..."`
      - ⚠️ 风险：`"intro_en": "It's a \"game changer\"..."`
